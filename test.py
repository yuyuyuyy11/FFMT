...
DATASET.add_argument('--name', type=str, default='ScanNet')
...
DATASET.add_argument('--batch_size', type=int, default= 4)
...
TEST.add_argument("--checkpoint", type=str, default='logs/experiments/URR3dmatch_1027-1544/best_loss.pkl')  #rewrite
TEST.add_argument("--dataset", type=str, default="RGBD_3DMatch", help=['ScanNet', 'RGBD_3DMatch'])      #RGBD_3DMatch
...
def evaluate_split(model, data_loader, args, dict_name=None, use_tqdm=True):
    ...
    # print('--',type(r_err),r_err.size(),r_err)
    r_err = r_err[~torch.isnan(r_err)]
    t_err = t_err[~torch.isnan(t_err)]
    c_err = c_err[~torch.isnan(c_err)]
    ...
    print(latex)
    try:
        with open('bash/output.log',"+a") as f:
            f.write('\n')
            f.write(latex)
            f.write('\n')
            f.write('\n')
