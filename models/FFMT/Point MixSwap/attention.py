...
if __name__ == '__main__':
    # 定义注意力机制
    block = Attention(dim_Q=128, dim_K=128, dim_LIN=32, num_heads=8, n_pts=128, ln=True)

    # 创建模拟输入数据
    batch_size = 6
    Q = torch.randn(batch_size, 32, 128) # Query 张量
    K = torch.randn(batch_size, 32, 128) # Key 张量
