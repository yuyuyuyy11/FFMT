...
def transform_points_Rt(
    ...
    '''
    该函数根据输入的位姿矩阵 viewpoint，对输入的三维点 points 进行坐标变换。位姿矩阵由一个3x3的旋转矩阵和一个1x3的平移向量组成。
如果 inverse=False，函数会执行标准的位姿变换：先对点进行旋转，再进行平移。
如果 inverse=True，函数会执行逆变换：先移除平移，再对点进行逆旋转。
这是一个常见的几何变换函数，通常用于将点从一个坐标系转换到另一个坐标系，广泛应用于计算机视觉、3D重建、SLAM等领域
